...
Die Funktion `text` ist im Modul `Html` definiert und nimmt einen `String` und liefert einen HTML-Text-Knoten.
Unter <https://package.elm-lang.org/packages/elm/html/latest/> findet sich eine Beschreibung des Moduls `Html`.
Wenn wir eine Definition aus dem Modul `Html` verwenden wollen, müssen wir es in der Liste hinter `exposing` aufführen.
...
Wenn wir auch die Konstruktoren von einem Datentyp wie `Html` verwenden möchten, müssen wir in der Liste nach `exposing` die Angabe `Html(..)` machen.
...
Es ist ein sehr probates Mittel, ein Elm-Modul mithilfe von Kommentaren zu strukturieren.
...
machen kann.
Das Modul `Debug` stellt eine Funktion
`log : String -> a -> a` zur Verfügung.
Wenn diese Funktion ausgewertet
wird, schreibt sie ihr zweites Argument auf die Konsole.
Der `String` im
ersten Argument wird dieser Ausgabe vorangestellt.
Wir nutzen in unserer
...
unseres Browsers an.
Wenn wir wiederholt auf den Knopf für das Erhöhen
...
hat.
Wenn wir auf den Knopf für das Verringern des Zählers drücken,
